...
class InfFS:
    ...
    def infFS(self,x_train, y_train, alpha, supervision, verbose):
        ...
        corr_ij_2, pval = stats.spearmanr(x_train)
        for i in range( 0,corr_ij_2.shape[0] ):
            for j in range( 0,corr_ij_2.shape[1] ):
                if( math.isnan(corr_ij_2[i,j]) or corr_ij_2[i,j] < -1 or corr_ij_2[i,j] > 1 ):
                    corr_ij_2[i,j] = 0
        ...
        A =  ( (alpha/2)*corr_ij + (1-alpha)*sigma_ij + (alpha/2)*corr_ij_2 );
